...
import {testLabelData1, testLabelData10, testLabelData10r, testTitle1, testTitle2} from "axis/AxisData.test";
...
describe("Axis", function() {
  ...
  for (let location in AxisLocation) {
    it(`Creates axis for location: ${AxisLocation[location]} (${location})`, function() {
      let at = new Axis({
  ...
  it(`Updates title`, function(){
    ...
    assert.equal(testTitle1, at.element.getElementsByClassName("pgAxisTitle")[0].textContent);
    at.title = testTitle2;
    assert.equal(testTitle2, at.element.getElementsByClassName("pgAxisTitle")[0].textContent);
  });
  ...
  it(`Flips label direction `, function(){
    ...
    let labelElements = at.element.getElementsByClassName("pgAxisLabels")[0].children;
    for (let i =0; i < labelElements.length; i++) {
      assert.equal(labelElements[i].textContent, testLabelData10.labels[i]);
    }

    at.flipDirection = true;
    labelElements = at.element.getElementsByClassName("pgAxisLabels")[0].children;
    for (let i =0; i < labelElements.length; i++) {
